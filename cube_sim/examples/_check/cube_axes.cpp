...
// ===========================================================================
// Cube Axes - generated by the WLED Effect Studio node editor
// ===========================================================================
...
static FX_RET mode_cube_axes() {
  ...
  for (int py = 0; py < H; py++) {
    ...
    for (int px = 0; px < W; px++) {
      ...
      uint32_t gc_out = 0;
      // Position #2
      GcVec n2_pos = GcVec{0.0f, 0.0f, 0.0f}; float n2_x = 0; float n2_y = 0; float n2_z = 0; 
      n2_pos = gc_v3(X3, Y3, Z3); n2_x = X3; n2_y = Y3; n2_z = Z3;
      // Remap #3
      float n3_result = 0; 
      n3_result = 0.0f + (n2_x - -1.0f) * ((1.0f - 0.0f) / ((1.0f - -1.0f) != 0.0f ? (1.0f - -1.0f) : 1.0f));
      // Remap #4
      float n4_result = 0; 
      n4_result = 0.0f + (n2_y - -1.0f) * ((1.0f - 0.0f) / ((1.0f - -1.0f) != 0.0f ? (1.0f - -1.0f) : 1.0f));
      // Remap #5
      float n5_result = 0; 
      n5_result = 0.0f + (n2_z - -1.0f) * ((1.0f - 0.0f) / ((1.0f - -1.0f) != 0.0f ? (1.0f - -1.0f) : 1.0f));
      // Combine #6
      uint32_t n6_color = 0; 
      n6_color = RGBW32((uint8_t)(gc_sat(n3_result) * 255.0f), (uint8_t)(gc_sat(n4_result) * 255.0f), (uint8_t)(gc_sat(n5_result) * 255.0f), 0);
      // Output #7
...
static const char _data_FX_MODE_CUBE_AXES[] PROGMEM = "Cube Axes@,,,,,,,;;!;2;sx=128,ix=128,pal=11";
static CfxBankReg cube_axes_reg(&mode_cube_axes, _data_FX_MODE_CUBE_AXES);
