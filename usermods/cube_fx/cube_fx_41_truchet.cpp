...
// two. So a kick flips a handful of tiles. That is a discrete, structural
// event rather than a brightness pulse, it is the only thing this pattern can
// do that nothing else in the set can, and it does not need easing: a Truchet
// tile has no meaningful half-way rotation, so the flip is instant and the
// flipped tile flashes briefly to say it was deliberate.
//
// ---------------------------------------------------------------------------
...
struct TrState {
  ...
  uint8_t  flash[TR_CELLS];     // decaying, set when a tile flips
};
...
static FX_RET mode_truchet() {
  ...
  { const int d = (int)fx_step(11, dt);
    for (int c = 0; c < nCell; c++)
      s->flash[c] = (uint8_t)((s->flash[c] > d) ? (s->flash[c] - d) : 0); }
  ...
  for (int y = 0; y < rows; y++) {
    ...
    for (int x = 0; x < cols; x++) {
      ...
      const float u = fa - (float)i, v = fb - (float)j;
      ...
      uint8_t idx = (uint8_t)((int)(uint8_t)(h >> 11) / 3 + hueOff);
...
static const char _data_FX_MODE_TRUCHET[] PROGMEM =
  "Ace 3-D Truchet@Flow,Fill,Cells,Weight,Style,Rewire on beat,Contour,Flat mode;;!;2f;sx=90,ix=190,c1=60,c2=110,c3=3,o1=1,pal=11";
