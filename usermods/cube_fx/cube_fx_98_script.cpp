...
static bool ssParse(SsProgram &P) {
  ...
  if (P.nf < SS_FIXED || P.nf > 4096 || P.nc > 2048 || P.ns > 4096) return false;
  P.frame = P.bytes + 19;
...
static uint32_t ssRun(const uint8_t *p, const uint8_t *end, float *F, uint32_t *C, float *S,
                      uint16_t nf, uint16_t nc, uint16_t ns, const uint8_t *fft) {
  uint32_t out = 0;
  #define RF(i) F[(i) < nf ? (i) : 0]
  #define RC(i) C[(i) < nc ? (i) : 0]
  while (p < end) {
    const uint8_t op = *p++;
    uint16_t a, b, c, d;
    switch (op) {
      ...
      case SS_SIN: a = ssU16(p); b = ssU16(p); RF(a) = sinf(RF(b)); break;
      case SS_COS: a = ssU16(p); b = ssU16(p); RF(a) = cosf(RF(b)); break;
      ...
      case SS_PAL: { a = ssU16(p); b = ssU16(p); c = ssU16(p);
        RC(a) = mq_scale(SEGMENT.color_from_palette((uint8_t)(int)(gc_sat(RF(b)) * 255.0f), false, true, 0),
                         (uint8_t)(gc_sat(RF(c)) * 255.0f)); break; }
...
static void mode_studio_script() {
  ...
  for (int py = 0; py < H; py++) {
    ...
    for (int px = 0; px < W; px++) {
      ...
      F[SSF_r] = sqrtf(cx * cx + cy * cy); F[SSF_ang] = cfx_atan2f(cy, cx);
