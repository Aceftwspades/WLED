#include "wled.h"
#include <math.h>
#include "cube_fx_common.h"   // shared fx_dt/fx_dt8/fx_step/fx_fade timing helpers
...
/*
 * ===========================================================================
 * The flat 2-D analysers
 * ===========================================================================
 * This file holds the seven flat 2-D effects together, for historical reasons;
 * every other effect in the folder lives in its own cube_fx_NN_name.cpp. See
 * README.md for the layout and how to add an effect.
 *
 ...
 * that signature will not compile here (set -D FX_LEGACY_RETURN=1 for 0.15).
 * ===========================================================================
 */
...
// fx_dt / fx_dt8 / fx_step / fx_fade now come from cube_fx_common.h - byte-for-
// byte the same helpers this file used to define on its own.

// --- beat gate ---------------------------------------------------------------
// fx_lowBeat() now lives in cube_fx_common.h (shared, proper `inline` linkage
// across the whole build - see the note there on why `static inline` in a
// header was wrong).
...
// REGISTRATION
// ===========================================================================
// Each effect joins the bank, which decides whether it claims one of the
// device's limited effect slots. See cube_fx_bank.h.
static CfxBankReg cube_fx_2d_reg_0(&mode_chladni,            _data_FX_MODE_CHLADNI);
static CfxBankReg cube_fx_2d_reg_1(&mode_spectral_rd,        _data_FX_MODE_SPECTRAL_RD);
static CfxBankReg cube_fx_2d_reg_2(&mode_harmonic_lissajous, _data_FX_MODE_HARMONIC_LISSAJOUS);
static CfxBankReg cube_fx_2d_reg_3(&mode_kaleidoscope,       _data_FX_MODE_KALEIDOSCOPE);
static CfxBankReg cube_fx_2d_reg_4(&mode_moire_rosette,      _data_FX_MODE_MOIRE_ROSETTE);
static CfxBankReg cube_fx_2d_reg_5(&mode_spectral_wormhole,  _data_FX_MODE_SPECTRAL_WORMHOLE);
static CfxBankReg cube_fx_2d_reg_6(&mode_glass_kaleidoscope, _data_FX_MODE_GLASS_KALEIDOSCOPE);
